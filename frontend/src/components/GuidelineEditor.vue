...
<template>
  <div class="bg-slate-800/80 backdrop-blur-md border border-slate-700 rounded-2xl p-6 shadow-xl">
    ...
    <div v-if="!isEditing" class="space-y-4">
      <div
        v-if="!props.initial"
        class="text-slate-500 bg-slate-900/30 p-6 rounded-xl text-center border border-slate-700/50 flex flex-col items-center justify-center gap-2"
      >
      ...
      <div v-else class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div class="bg-slate-900/50 p-4 rounded-xl border border-slate-700/50">
          <label class="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2"
            >Sensitive Topics</label
          >
          <div class="flex flex-wrap gap-2">
            <span v-if="!props.initial.sensitive_topics?.length" class="text-sm text-slate-500"
              >Not set</span
            >
            <span
              v-for="topic in props.initial.sensitive_topics"
              :key="topic"
              class="text-xs px-2.5 py-1 bg-slate-800 text-slate-300 rounded-md border border-slate-700"
              >{{ topic }}</span
            >
          </div>
        </div>
        <div class="bg-slate-900/50 p-4 rounded-xl border border-slate-700/50 md:col-span-2">
          ...
          <p class="text-sm text-slate-200 whitespace-pre-wrap leading-relaxed">
            {{ props.initial.coping_strategies || 'Not set' }}
          </p>
        </div>
        <div class="bg-slate-900/50 p-4 rounded-xl border border-slate-700/50 md:col-span-2">
          <label class="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2"
            >Behavioral Boundaries</label
          >
          <p class="text-sm text-slate-200 whitespace-pre-wrap leading-relaxed">
            {{ props.initial.behavioral_boundaries || 'Not set' }}
          </p>
        </div>
      </div>
    </div>
    ...
    <div v-else class="space-y-5">
      <div>
        ...
        <input
          ...
          class="w-full bg-slate-900/50 border border-slate-600 text-slate-200 rounded-lg p-3 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition-colors"
          placeholder="e.g. Empathetic, Direct"
        />
      </div>
      ...
      <div>
        ...
        <textarea
          v-model="coping"
          class="w-full bg-slate-900/50 border border-slate-600 text-slate-200 rounded-lg p-3 h-24 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition-colors resize-none scrollbar-thin scrollbar-thumb-slate-600"
        ></textarea>
      </div>
      ...
      <div>
        <label class="block text-sm font-medium text-slate-300 mb-2"
          >Sensitive Topics (comma-separated)</label
        >
        <textarea
          v-model="sensitive"
          class="w-full bg-slate-900/50 border border-slate-600 text-slate-200 rounded-lg p-3 h-20 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition-colors resize-none scrollbar-thin scrollbar-thumb-slate-600"
        ></textarea>
      </div>
      ...
      <div class="flex gap-3 pt-4 border-t border-slate-700/50">
        <button
          @click="save"
          class="px-6 py-2 bg-indigo-600 font-medium text-white rounded-lg hover:bg-indigo-500 transition-colors shadow-lg shadow-indigo-500/25"
        >
          Save Changes
        </button>
        <button
          @click="cancel"
          class="px-6 py-2 bg-slate-700 font-medium text-slate-300 rounded-lg hover:bg-slate-600 hover:text-white transition-colors"
        >
          Cancel
        </button>
      </div>
