...
const props = defineProps<{ progress: Array<{ metric_date: string; session_count: number }> }>()

const labels = computed(() => props.progress.map((entry) => new Date(entry.metric_date).toLocaleDateString()))
