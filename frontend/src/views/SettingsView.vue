...
<template>
  <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
    ...
    <section
      class="bg-slate-800/80 backdrop-blur-md border border-slate-700 rounded-2xl p-6 shadow-xl flex flex-col"
    >
      <h2
        class="text-xl font-bold mb-4 text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 to-cyan-400"
      >
        Collaboration
      </h2>
      <div class="flex gap-3 mb-4">
        ...
        />
        <button
          class="px-6 py-2.5 bg-cyan-600 hover:bg-cyan-500 text-white font-medium rounded-lg shadow-lg shadow-cyan-500/25 transition-all duration-200 transform hover:-translate-y-0.5"
          @click="addInvite"
        >
          Invite
        </button>
      ...
      <p
        v-if="inviteError"
        class="text-red-400 text-sm mb-4 bg-red-900/20 border border-red-500/20 p-2 rounded-lg"
      >
        {{ inviteError }}
      </p>
      ...
      <div class="flex-1 overflow-hidden flex flex-col">
        ...
        <ul class="space-y-3 overflow-y-auto scrollbar-thin scrollbar-thumb-slate-600 pr-2">
          <li
            v-for="invite in dashboard.invitations"
            :key="invite.id"
            class="flex justify-between items-center p-3 bg-slate-900/30 border border-slate-700/50 rounded-xl hover:bg-slate-700/30 transition-colors"
          >
            <div>
            ...
            <button
              class="w-8 h-8 flex items-center justify-center text-slate-400 hover:text-red-400 hover:bg-red-400/10 rounded-full transition-colors"
          ...
          <li
            v-if="dashboard.invitations.length === 0"
            class="text-slate-500 text-sm py-4 text-center"
          >
