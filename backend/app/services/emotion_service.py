...
""" % content
    ...
    try:
        ...
        return _safe_emotion_result(response.content)
    except Exception:
...
async def upsert_emotion_snapshot(session_id, db):
    stmt = select(Message).where(Message.session_id == session_id, Message.sender == "user")
    ...
    upsert = insert(EmotionSnapshot).values(
...
async def upsert_dashboard_summary(patient_id, session_id, created_at, db):
    ...
    session_today_stmt = select(func.count()).select_from(Message).where(
        Message.sender == "user",
    ...
    upsert = insert(DashboardSummary).values(
        user_id=patient_id,
        summary_date=today,
